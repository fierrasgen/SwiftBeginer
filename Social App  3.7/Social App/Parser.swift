...
class Photo: Object, Decodable {
    ...
    var url: String {
            return "https://api.vk.com/method/photos.get?owner_id=\(UserSession.instance.userId)"
        }
...
class Post: Object, Decodable {
    ...
    required init(from decoder: Decoder) throws {
        ...
        let dateType  = try decoder.container(keyedBy: NewsKeys.self)
        self.date = try dateType.decode(Double.self, forKey: .date)
        let sourceType = try decoder.container(keyedBy: NewsKeys.self)
        self.sourceId = try sourceType.decode(Int.self, forKey: .sourceId)
        let source = try decoder.container(keyedBy: NewsKeys.self)
        self.source = try source.decode(String.self, forKey: .source)
    }
}
